...
const ChatRoom: React.FC = () => {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    e.preventDefault();
    if (inputText.trim() && ownKeyPair && peerPublicKeyBase64) { 
      sendMessage(inputText.trim());
      setInputText('');
    } else if (!isPeerConnected) {
        addSystemMessageUI("Cannot send: Peer's public key not yet exchanged.");
    } else {
        // This case should ideally not be hit if disabled logic is correct
        addSystemMessageUI("Cannot send: Keys or connection issue.");
    }
  };

  // Helper to add system messages directly from UI if needed (though most come from context)
  const addSystemMessageUI = (text: string, type: SystemMessageType = SystemMessageType.ERROR) => {
    // This function is illustrative; prefer centralizing system messages in ChatContext
    // For now, this could console.warn or be a no-op if context handles all user-facing system messages
  ...
  const handleSetRemoteOffer = () => {
    if (manualSdp.trim()) setRemoteSdp(manualSdp.trim(), 'offer');
    setManualSdp('');
  };
  const handleSetRemoteAnswer = () => {
     if (manualSdp.trim()) setRemoteSdp(manualSdp.trim(), 'answer');
     setManualSdp('');
  };
  const handleAddManualIce = () => {
    if (manualIce.trim()) addRemoteIceCandidate(manualIce.trim());
    setManualIce('');
  };
  ...
  return (
    <div className="w-full max-w-3xl h-[calc(100vh-150px)] flex flex-col p-4 sm:p-6 bg-gray-800 rounded-xl shadow-2xl">
      <div className="mb-4">
        ...
        {isPeerConnected && !isWebRTCConnected && (
            <button
      ...
      <form onSubmit={handleSendMessage} className="flex items-center space-x-2">
        <input
          ...
          placeholder={isPeerConnected ? (isWebRTCConnected ? "Type (WebRTC P2P)..." : "Type (BroadcastChannel fallback)...") : "Waiting for peer connection..."}
